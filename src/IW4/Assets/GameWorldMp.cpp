...
namespace ZoneTool
{
	namespace IW4
	{
		H1::GlassWorld* GenerateH1GlassWorld(IW5::GlassWorld* asset, ZoneMemory* mem)
		{
		...
		// IW4's GameWorldMp is IW5's GlassWorld (and so is GameWorldSp but it's not 1:1)
		void IGameWorldMp::dump(GameWorldMp* asset, ZoneMemory* mem)
		{
			// generate h1 glassworld
			auto h1_asset = GenerateH1GlassWorld(reinterpret_cast<IW5::GlassWorld*>(asset), mem);
