...
namespace ZoneTool
{
	...
	namespace T6
	{
		H1::GfxImage* GenerateH1GfxImage(GfxImage* asset, allocator& allocator)
		{
			// allocate H1 GfxImage structure
			const auto h1_asset = allocator.allocate<H1::GfxImage>();

			h1_asset->name = asset->name;
			h1_asset->imageFormat = directx1::get_d3d_to_dxgi(asset->texture.loadDef->format);
			...
			if (asset->texture.loadDef->format == directx1::D3DFMT_A8R8G8B8 && h1_asset->imageFormat == DXGI_FORMAT_R8G8B8A8_UNORM)
			{
				auto* new_pixels = allocator.allocate<unsigned char>(asset->texture.loadDef->resourceSize);
		...
		void IGfxImage::dump(GfxImage* asset)
		{
			...
			if (!asset->texture.loadDef || !asset->texture.loadDef->resourceSize)
			{
			...
			auto h1_asset = GenerateH1GfxImage(asset, allocator);
