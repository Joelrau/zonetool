...
{
	// The probe index in indexFlags is wider than 16 bits: mp_paris ships 114336 probes and its
	// indexFlags reach 114335, which needs 17. Bits 17..30 are zero in every shipped map and bit 31
	// is the corner flag, so 17 bits is what the shipped data actually demonstrates - cap there
	// rather than assuming the whole 31-bit field is addressable.
	//
	// Note this bounds memory as well as indices: we emit a tetrahedronVisibility entry for every
	// tetrahedron at 64 bytes each, so a map that actually reaches this cap costs ~50 MB in that
	// table alone (mp_paris ships 297161 entries, 19 MB). Raise the cap further only alongside a
	// smarter visibility table.
	...
	struct build_params
	{
		...
		// Optional test for whether a leaf cell should exist at all, given its world-space corner
		// and edge length. Shipped volumes are sparse and shaped like the map - probes exist only
		// where there is open space, so a model next to geometry interpolates against genuinely
		// dark probes. Filling the whole bounding box instead (what this generator did originally)
		// gives every cell a plausible mid-grey from the nearest lit sample, which reads as "lit
		// from every direction": correct average colour, no contrast, no form. Leave empty to keep
		// the old fill-everything behaviour.
	...
	struct probe_volume
	{
		...
		// World-space placement of each leaf, which the tree itself does not carry: leaf i spans
		// [leaf_bounds_min[i*3 .. +2], + leaf_size) on each axis. The tree stores only a child mask
		// per node, so a leaf's position is implied by the descent that reached it and is not
		// recoverable from the emitted arrays. A caller that wants to know which lights reach which
		// voxel needs the boxes, so they are handed out alongside.
		std::vector<float> leaf_bounds_min;          // xyz per leaf
		float leaf_size = 0.0f;

		bool valid = false;
	};

	// Lighting at a world position: 28 floats. The first 27 are L2 spherical harmonics, three
	// blocks of nine (all R, then all G, then all B). [27] is the probe's **sun/sky visibility**
	// in [0, 1], which every shipped map varies per probe - dome_dusk's median is 0.0002 and
	// mp_frontend's is 0.99, exactly what a baked occlusion term looks like on a dusk map versus
	// an open lobby. Writing a constant 1.0 there (what this generator did originally) tells the
	// shader every probe sees the full sun with no occlusion, which flattens every model in the
	// map. Only the zone fallback is legitimately 1.0. Called once per probe.
	...
	// packs the 27 coefficients plus the visibility term at [27]; [28..31] stay zero, as in every
	// shipped map
