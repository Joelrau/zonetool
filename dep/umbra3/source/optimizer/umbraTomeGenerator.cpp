...
#ifdef UMBRA_IW7_CELL_EXIT_PORTALS
        // ZoneTool: Call of Duty: Infinite Warfare's Umbra runtime does not seed the
        // camera's cell with full screen coverage the way this SDK's runtime does
        // (PortalCuller::addStartCell); coverage there only ever comes from portal
        // rectangles. A cell face with no portal - a solid floor or wall - leaves the
        // part of the screen it occupies uncovered, and anything the engine tests
        // against the occlusion buffer in that region (brush models, scene entities,
        // reflection probes) is culled. An exit portal spanning each whole cell face
        // gives the interior of every cell "outside" coverage at maximum depth, which
        // is exactly the full screen seed and can only ever add visibility. Exit
        // portals lead nowhere, so the traversal cost is a rectangle per face, and
        // both runtimes honour them on any face when no tome matching data is present.
        {
            const AABB& tileAABB = tile->getAABB();
            AABB cellAABB = cell.getAABB();
            for (int a = 0; a < 3; a++)
            {
                cellAABB.setMin(a, max2(cellAABB.getMin()[a], tileAABB.getMin()[a]));
                cellAABB.setMax(a, min2(cellAABB.getMax()[a], tileAABB.getMax()[a]));
            }
            ...
            for (int face = 0; face < 6; face++)
            {
                ...
                int axisX = (axis + 1) % 3;
                int axisY = (axis + 2) % 3;
                if (!(cellAABB.getMin()[axisX] < cellAABB.getMax()[axisX]) ||
                    !(cellAABB.getMin()[axisY] < cellAABB.getMax()[axisY]))
                    continue;

                Vector3 mn = cellAABB.getMin();
                Vector3 mx = cellAABB.getMax();
                float z = getFaceDirection(face) ? mx[axis] : mn[axis];
                mn[axis] = z;
                mx[axis] = z;

                Portal portal;
                initPortal(portal, BUILD_PORTAL_LINK(face, 1, 0, 0, Portal::getMaxSlotIdx()), 0, AABB(mn, mx), tileAABB);
